...
Function Export-AzSentinelConfigurationToWord($fileName) {
    ...
    try {
        ...
        if ($ShowWorkbooks.checked) {
            Add-AllWorkbooks $metadata $selection 
            $selection.InsertBreak(7)   #page break
        }
        ...
        if ($ShowDataConnectors.checked) {
            Add-AllDataConnectors $solutions $selection
            $selection.InsertBreak(7)   #page break
        }
...
Function Add-AllTables ( $selection) {
    $selection.Style = "Heading 1"
    $selection.TypeText("Tables")
    $selection.TypeParagraph()
    ...
    try {
        ...
        foreach ($laTable in $laTables | Sort-Object { $_.properties.schema.name }) {
            ...
            $selection.TypeText($laTable.properties.schema.name.toString())
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tPlan: ");
            $selection.Font.Bold = $false
            $selection.TypeText($laTable.properties.plan.toString())
            #$selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tInteractive Retention: ");
            $selection.Font.Bold = $false
            $selection.TypeText($laTable.properties.retentionInDays.toString())
            #$selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tTotal Retention: ");
            $selection.Font.Bold = $false
            $selection.TypeText($laTable.properties.totalRetentionInDays.toString())
            $selection.TypeParagraph()
...
Function Add-AllWorkspaces ( $selection) {
    $selection.Style = "Heading 1"
    $selection.TypeText("Workspaces")
    $selection.TypeParagraph()
    ...
    try {
        ...
        "&canFetchContent=false&%24filter=sourceId%20eq%20'%2Fsubscriptions%2F$SubscriptionId%2FresourceGroups%2F$resourceGroupName%2F" +
        "providers%2FMicrosoft.OperationalInsights%2Fworkspaces%2F$workspaceName'&category=sentinel"
        ...
        foreach ($workspace in $workspaces.data.rows | Sort-Object { $_.properties.displayName }) {
            # $selection.Style = "Heading 2"
            $selection.Font.Bold = $false
            $selection.TypeText($workspace[0])
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tGroup(s): ");
            $selection.Font.Bold = $false
            $selection.TypeParagraph()
                ...
                foreach ($resourceId in $resourceIds) {
                    ...
                    if ($null -ne $match) { 
                        ...
                        if ($name -eq $workspace[0]) {
                            ...
                            $selection.TypeText($group.properties.displayName);
                            $selection.Font.Bold = $false
                            $selection.TypeParagraph()
                        }
                    }
            ...
            $selection.Font.Bold = $true
            $selection.TypeText("`tDirectory: ");
            $selection.Font.Bold = $false
            if ($null -eq $tenant.displayName) {
                $selection.TypeText("unknown")
            }
            else {
                $selection.TypeText($tenant.displayName)
            }
            
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tLocation: ");
            $selection.Font.Bold = $false
            $selection.TypeText($workspace[2])
            $selection.TypeParagraph()
        }
        ...
        foreach ($group in $groups) {
            ...
            $selection.Font.Bold = $true
            $selection.TypeText("`tDescription: ");
            $selection.Font.Bold = $false
            $selection.TypeText($group.properties.description)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tWorkspaces: ");
            $selection.Font.Bold = $false
            $selection.TypeText($group.properties.memberResourceNames.count)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tContent Items: ");
            $selection.Font.Bold = $false
            $selection.TypeText($assignmentList.properties.items.count)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tLast Publish Status: ");
            $selection.Font.Bold = $false
            if ($null -ne $assignmentList.properties.LastJobProvisioningState) {
                $selection.TypeText($assignmentList.properties.LastJobProvisioningState)
            }
            else {
                $selection.TypeText("-")
            }
            ...
            $selection.Font.Bold = $true
            $selection.TypeText("`tLast Publish Date: ");
            $selection.Font.Bold = $false
            if ($lastEndDateFormat -ne "") {
                $selection.TypeText($lastEndDateFormat.ToString("MM/dd/yyyy hh:mm tt"))
            }
            else {
                $selection.TypeText("-")
            }
            ...
            $selection.Font.Bold = $true
            $selection.TypeText("`tSelected Content: ");
            $selection.Font.Bold = $false
            $selection.TypeParagraph()
            ...
            foreach ($item in $assignmentList.properties.items) {
                ...
                if ($null -eq $found) {
                    $selection.Font.Bold = $true
                    $selection.TypeText("`t`tError matching: ");
                    $selection.Font.Bold = $false
                    $selection.TypeText($resourceId)
                    $selection.TypeParagraph()
                }
                else {
                    $selection.Font.Bold = $true
                    $selection.TypeText("`t`t");
                    $selection.Font.Bold = $false
                    $selection.TypeText($found.properties.displayName)
                    $selection.TypeParagraph()
...
Function Add-AllRBAC( $workbook) {
    ...
    try {
        ...
        foreach ($roleAssignment in $roleAssignments | Sort-Object { $_.properties.condition } ) {
            ...
            $StatusBox.text += "."
            $selection.TypeText($user.displayName)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tType: ");
            $selection.Font.Bold = $false
            $userType="User"
            ...
            $selection.TypeText($userType);
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tRole: ");
            $selection.Font.Bold = $false
            $selection.TypeText($roleAssignment.properties.condition);
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tScope: ");
            $selection.Font.Bold = $false
        
            switch ([string]($roleAssignment.properties.scope.split("/").count)) {
                "2" {
                    $selection.TypeText("Root")
                }
                "3" {
                    $selection.TypeText("Subscription (Inherited)")
                }
                "4" {
                    $selection.TypeText("This resource group")
                }
                default {
                    $selection.TypeText("Management Group")
                }
            }
            $selection.TypeParagraph()
        }
...
Function Add-AllSummaryRules ( $selection) {
    $selection.Style = "Heading 1"
    $selection.TypeText("Summary Rules")
    $selection.TypeParagraph()
    ...
    try {
        ...
        foreach ($rule in $summaryRules | Sort-Object { $_.properties.displayName }) {
            ...
            $selection.Font.Bold = $false
            $selection.TypeText($rule.properties.displayName)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("Description: ")
            $selection.Font.Bold = $false
            $selection.TypeText($rule.properties.description)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tFrequency: ")
            $selection.Font.Bold = $false
            $selection.TypeText($rule.properties.ruleDefinition.binsize)
            $selection.TypeText(" minutes")
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tStatus: ")
            $selection.Font.Bold = $false
            $selection.TypeText($rule.properties.isActive)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tProvisioning State: ")
            $selection.Font.Bold = $false
            $selection.TypeText($rule.properties.provisioningState )
            $selection.TypeParagraph()

            $filteredData = $queryRows | Where-Object { $_[0] -eq $rule.Name }

            $selection.Font.Bold = $true
            $selection.TypeText("`tLast Run Time: ")
            $selection.Font.Bold = $false
            $selection.TypeText($filteredData[1] )
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tLast run row count: ")
            $selection.Font.Bold = $false
            $selection.TypeText($filteredData[2] )
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tLast run status: ")
            $selection.Font.Bold = $false
            $selection.TypeText($filteredData[3] )
            $selection.TypeParagraph()
...
$tacticDescriptionHash = @{
    ...
    "ImpairProcessControl"    = "The adversary is trying to manipulate, disable, or damage physical control processes."
    "InhibitResponseFunction" = "The adversary is trying to prevent your safety, protection, quality assurance, and operator intervention functions from responding to a failure, hazard, or unsafe state."
...
Function Add-AllMitre ( $selection, $word) {

    $selection.Style = "Heading 1"
    $selection.TypeText("MITRE Coverage")
    $selection.TypeParagraph()

    Write-Host "MITRE coverage: " 
    ...
    try {
        ...
        foreach ($tactic in $tacticsAndTechniques.Keys) {
            $StatusBox.text += "."
            $selection.Style = "Heading 2"
            $selection.TypeText("Tactic: ")
            $selection.Font.Bold = $false
            $selection.TypeText($tacticNameHash[$tactic])
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("Description: ")
            $selection.Font.Bold = $false
            $selection.TypeText( $tacticDescriptionHash[$tactic])
            $selection.TypeParagraph()
            ...
            foreach ($technique in $tacticsAndTechniques[$tactic].Keys) {
                if ("none" -eq $technique) {
                    $StatusBox.text += "."
                    $selection.Font.Bold = $true
                    $selection.TypeText("`tTechnique: ")
                    $selection.Font.Bold = $false
                    $selection.TypeText("none selected")
                    $selection.TypeText(" ")
                    $selection.Font.Bold = $true
                    $selection.TypeText("`tCount: ")
                    $selection.Font.Bold = $false
                    $selection.TypeText($tacticsAndTechniques[$tactic][$technique])
                    $selection.TypeParagraph()

                    $selection.Font.Bold = $true
                    $selection.TypeText("`tName: ")
                    $selection.Font.Bold = $false
                    $selection.TypeText("No technique selected")
                    $selection.TypeParagraph()

                    $selection.Font.Bold = $true
                    $selection.TypeText("`tDescription: ")
                    $selection.Font.Bold = $false
                    $selection.TypeText("No description available")
                    $selection.TypeParagraph()
                    $selection.TypeParagraph()
                }
                else {
                    ...
                    $selection.TypeText("`tName: ")
                    $selection.Font.Bold = $false
                    $selection.TypeText($techniqueNameHash[$technique] )
                    $selection.TypeParagraph()

                    $selection.Font.Bold = $true
                    $selection.TypeText("`tDescription: ")
                    $selection.Font.Bold = $false
                    $selection.TypeText($techniqueDescriptionHash[$technique] )
                    $selection.TypeParagraph()
                    $selection.TypeParagraph()
                }
...
Function Add-AllAutomation ($alertRules, $selection) {
...
Function Add-AllPlaybooks ($selection) {
    $selection.Style = "Heading 1"
    ...
    try {
        ...
        foreach ($playbook in $consumptionPlaybooks.data.rows) {
            ...
            if ($playbook[4].definition.triggers -match "Microsoft_Sentinel") {
                #$selection.Style = "Heading 2"
                $selection.TypeText($playbook[1])
                $selection.TypeParagraph()

                $selection.Font.Bold = $true
                $selection.TypeText("`tStatus: ");
                $selection.Font.Bold = $false
                $selection.TypeText($playbook[4].state);
                $selection.TypeParagraph()

                $selection.Font.Bold = $true
                $selection.TypeText("`tPlan: ");
                $selection.Font.Bold = $false
                $selection.TypeText("Consumption");
                $selection.TypeParagraph()
                ...
                $selection.TypeText("`tTrigger kind: ");
                $selection.Font.Bold = $false
                switch ($triggerKey) {
                    "Microsoft_Sentinel_incident" { $selection.TypeText("Microsoft Sentinel Incident") }
                    "Microsoft_Sentinel_alert" { $selection.TypeText("Microsoft Sentinel Alert") }
                    "Microsoft_Sentinel_entity" { $selection.TypeText("Microsoft Sentinel Entity") }
                    Default { $selection.TypeText("Unknown") }
                }
                $selection.TypeParagraph()

                $selection.Font.Bold = $true
                $selection.TypeText("`tResource group: ");
                $selection.Font.Bold = $false
                $selection.TypeText($playbook[0].split('/')[4]);
                $selection.TypeParagraph()

                $selection.Font.Bold = $true
                $selection.TypeText("`tTags: ");
                $selection.TypeParagraph()
                $selection.Font.Bold = $false
                if ($null -ne $playbook[5]) {
                    ...
                    for ($i = 0; $i -le $keys.count - 1; $i++) {
                        if ($keys[$i] -notmatch "hidden") {
                            $selection.TypeText("`t`t" + $keys[$i] + ": " + $values[$i])
                            $selection.TypeParagraph()
                        }
                    }
                }
        ...
        foreach ($playbook in $standardPlaybooks.data.rows) {
            ...
            if ($playbook[4].Files."workflow.json".definition.triggers -match "Microsoft_Sentinel") {
                ...
                $selection.TypeText("`tStatus: ");
                $selection.Font.Bold = $false
                $selection.TypeText($playbook[4].FlowState);
                $selection.TypeParagraph()

                $selection.Font.Bold = $true
                $selection.TypeText("`tPlan: ");
                $selection.Font.Bold = $false
                $selection.TypeText("Standard");
                $selection.TypeParagraph()
        ...
        $errorReturn = $_
        Write-Error $errorReturn
        $StatusBox.text += "Error occurred rendering Pkaybooks: " + $errorReturn
        $StatusBox.text += "`r`n"
    }
}
...
Function Add-AllWatchlists ($selection) {
    ...
    try {
        ...
        foreach ($watchlist in $watchlists | Sort-Object { $_.properties.displayName }) {
            ...
            #$selection.Style = "Heading 2"
            $selection.Font.Bold = $false
            $selection.TypeText($watchlist.properties.displayName)
            $selection.TypeParagraph()
            $selection.Font.Bold = $true
            $selection.TypeText("`tAlias: ");
            $selection.Font.Bold = $false
            $selection.TypeText($watchlist.properties.watchlistAlias)
            $selection.TypeParagraph()
            $selection.Font.Bold = $true
            $selection.TypeText("`tSource: ");
            $selection.Font.Bold = $false
            $selection.TypeText($watchlist.properties.source)
            $selection.TypeParagraph()
            $selection.Font.Bold = $true
            $selection.TypeText("`tCreated Time: ")
            $selection.Font.Bold = $false
            $selection.TypeText($watchlist.properties.created.toString())
            $selection.TypeParagraph()
            $selection.Font.Bold = $true
            $selection.TypeText("`tLast Updated: ")
            $selection.TypeText($watchlist.properties.updated.toString())
            $selection.Font.Bold = $false
            $selection.TypeParagraph()
        }
        $selection.endKey(6) | Out-Null
...
Function Add-AllAnalyticRules ($analyticRules, $selection) {
    ...
    try {
        ...
        foreach ($analyticRule in $analyticRules) {
            #$selection.Style = "Heading 2"
            $selection.Font.Bold = $false
            $selection.TypeText($analyticRule.properties.displayName)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tSeverity: ");
            $selection.Font.Bold = $false
            $selection.TypeText($analyticRule.properties.Severity)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tRule Type: ");
            $selection.Font.Bold = $false
            $selection.TypeText($analyticRule.kind)
            $selection.TypeParagraph()
            ...
            if ($analyticRule.properties.enabled -eq "True") {
                ...
                $selection.TypeText("Disabled")
            }
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tTactics: ");
            $selection.Font.Bold = $false
            $tactics = "`n"
            foreach ($tactic in $analyticRule.properties.tactics) {
                $tactics = $tactics + "`t`t" + $tactic + "`n"
            }
            $selection.TypeText($tactics)
            $selection.TypeParagraph()

            $selection.Font.Bold = $true
            $selection.TypeText("`tTechniques: ");
            $selection.Font.Bold = $false
            $techniques = "`n"
            foreach ($technique in $analyticRule.properties.Techniques) {
                $techniques = $techniques + "`t`t" + $technique + "`n"
            }
...
Function Add-AllDataConnectors ($solutions, $selection) {
    $selection.Style = "Heading 1"
...
Function Add-AllHunts($selection) {
    ...
    try {
        ...
        foreach ($hunt in $hunts | Sort-Object { $_.properties.displayName }) {
            ...
            $selection.TypeParagraph()
            $selection.TypeText("`tStatus: " + $hunt.properties.status);
            $selection.TypeText("`r`n`tHypothesis: " + $hunt.properties.hypothesisStatus);
            #$selection.TypeParagraph()
            if ($null -ne $hunt.properties.owner.assignedTo) {
                $selection.TypeText("`r`n`tAssigned To: " + $hunt.properties.owner.assignedTo);
            }
            else {
                $selection.TypeText("`r`n`tAssigned To: <unassigned>");
            }
            $selection.TypeParagraph()
            Write-Host "." -NoNewline
            $StatusBox.text += "."
...
Function Add-AllWorkbooks ( $metadata, $selection) {
    $selection.Style = "Heading 1"
    ...
    try {
        ...
        foreach ($workbook in $customWorkbooks | Sort-Object { $_.properties.displayName }) {
            #$selection.Style = "Heading 2"
            $selection.Font.Bold = $false
            $selection.TypeText($workbook.properties.displayName)
            $selection.TypeParagraph()
            ...
            if ($null -ne $foundTemplate) {
                $selection.TypeText($foundTemplate.properties.source.name)
            }
            else {
                $selection.TypeText("Custom")
            }
            $selection.TypeParagraph()
            Write-Host "." -NoNewline
            $StatusBox.text += "."
        ...
        foreach ($singleTemplate in $workbookTemplates | Sort-Object { $_.properties.displayName }) {
            ...
            #$selection.Style = "Heading 2"
            $selection.Font.Bold = $false
            $selection.TypeText($singleTemplate.properties.displayName)
            $selection.TypeParagraph()
            ...
            $selection.TypeText("`tSource: ");
            $selection.Font.Bold = $false
            $selection.TypeText($singleTemplate.properties.packageKind)
            $selection.TypeParagraph()
...
Function Show-UserInterface() {
    ...
    $HeaderLabel.Text = "Microsoft Sentinel Documentation Word Generator"
    ...
    $FileName.Text = "MicrosoftSentinelReport.docx"
    ...
    $CreateButton.Add_Click({
            Export-AzSentinelConfigurationToWord $FileName.text
        })
